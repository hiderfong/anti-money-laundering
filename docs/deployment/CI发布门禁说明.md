...
当前云端主发布门禁为 GitHub Actions，主工作流文件为 `.github/workflows/ci.yml`。本机 Gitea 仍保留 `.gitea/workflows/ci.yml`，用于本机/局域网 CI 备份与离线验证。

本机 Gitea 的 job 容器无法稳定访问以 `localhost` 生成的 artifact 上传地址，因此 `.gitea/workflows/ci.yml` 不上传 artifact，而是在日志中列出后端测试报告、前端构建产物和浏览器 E2E 截图路径。GitHub Actions 工作流保留 artifact 上传。
...
Docker 镜像构建使用 BuildKit Maven cache，不再单独执行 `dependency:go-offline`。这样可以避免冷启动 runner 在 Docker 构建阶段重复长时间解析依赖，同时仍通过 `scripts/ci-maven.sh clean package -DskipTests` 校验镜像内可完成应用打包。本地复现容器构建时需确认 Docker CLI 已安装并启用 `buildx`/BuildKit。
